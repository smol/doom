...
namespace Engine {
	export class Earcut {
		...
		constructor(data, holeIndices, dim) {
			dim = dim || 2;
			...
			this.earcutLinked(outerNode, this.triangles, dim, minX, minY, invSize, false);
		}
		...
		private filterPoints(start, end) {
		...
		private earcutLinked(ear, triangles, dim, minX, minY, invSize, pass) {
			if (!ear) return;
			...
			while (ear.prev !== ear.next) {
				...
				if (ear === stop) {
					...
					if (!pass) {
						this.earcutLinked(this.filterPoints(ear, null), triangles, dim, minX, minY, invSize, 1);
			...
			do {
				...
				while (b !== a.prev) {
					if (a.i !== b.i && this.isValidDiagonal(a, b)) {
						...
						this.earcutLinked(a, triangles, dim, minX, minY, invSize, false);
						this.earcutLinked(c, triangles, dim, minX, minY, invSize, false);
						return;
					}
					b = b.next;
				}
